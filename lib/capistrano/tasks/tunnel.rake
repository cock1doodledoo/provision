namespace :tunnel do
  ...
  namespace :ssh do
    ...
    task :unbind do
      on roles(:all) do |host|
        run_locally do
          proc = capture "ps ax | grep \"ssh -fN -R 8080:localhost:3128 "\
                         "#{host.user}@#{host.hostname}\"| head -1"
  ...
  namespace :wget do
    ...
    task :configure do
      on roles(:all) do
        execute :sudo, :chmod, 666, '/etc/wgetrc'
        execute 'echo http_proxy=http://localhost:8080/ >> /etc/wgetrc'
      end
    end
    ...
    task :unconfigure do
      on roles(:all) do
        ...
        execute :sudo, :chmod, 644, '/etc/wgetrc'
      end
    end
  end
end
